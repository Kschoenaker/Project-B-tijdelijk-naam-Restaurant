...
public class TableRecordsAccess
{
    ...
    {
        string sql = $@"INSERT INTO {Table} (Tables_ID, Reservation_ID) VALUES (@Tables_ID, @Reservation_ID)";
    ...
    {
        string sql = $@"UPDATE {Table} SET Tables_ID = @Tables_ID,Reservation_ID = @Reservation_ID WHERE ID = @ID";
        _connection.Execute(sql, tableRecord);
    }
